...
export default function GithubPulse() {
  ...
  const generateFallbackWeeks = (): ContributionDay[][] => {
    ...
    for (let w = 0; w < 26; w++) {
      ...
      for (let d = 0; d < 7; d++) {
        ...
        if (rand > 0.85) { level = "FOURTH_QUARTILE"; count = 12; }
        else if (rand > 0.7) { level = "THIRD_QUARTILE"; count = 6; }
        else if (rand > 0.5) { level = "SECOND_QUARTILE"; count = 3; }
        else if (rand > 0.35) { level = "FIRST_QUARTILE"; count = 1; }
  ...
  const fetchGithubData = async () => {
    ...
    try {
        ...
        const parsedActivities: GitActivity[] = events
          ...
          .map((event: any) => {
            ...
            return {
              id: event.id,
              type,
              repo: event.repo.name.replace("harsh-bhadana/", ""),
              message,
              time: timeFormatted,
      ...
      // 2. Fetch live contribution history graph (scraped from GitHub profile)
      const graphResponse = await fetch("https://github-contributions-api.deno.dev/harsh-bhadana.json");
      if (graphResponse.ok) {
        const graphData = await graphResponse.json();
        if (graphData.contributions) {
          // Get the last 26 weeks (representing the last 6 months)
          const last26Weeks = graphData.contributions.slice(-26);
          setWeeks(last26Weeks);
          
          // Calculate total contributions in this 6 month window
      ...
      console.warn("GitHub Fetch Error, loading fallback:", err.message);
  ...
  const mapLevelToColor = (level: string) => {
    switch (level) {
      case "FIRST_QUARTILE": return "#0e4429"; // Level 1
      case "SECOND_QUARTILE": return "#006d32"; // Level 2
      case "THIRD_QUARTILE": return "#26a641"; // Level 3
      case "FOURTH_QUARTILE": return "#39d353"; // Level 4
      default: return "#ebedf0"; // Level 0 (None) (standard GitHub light grey)
    }
  };
